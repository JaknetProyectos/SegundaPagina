...
export default function AdminDashboardPage() {
  ...
  const [selectedBookingItems, setSelectedBookingItems] = useState<any[]>([]);
  ...
  const handleOpenDetails = async (bookingId: string) => {
    ...
    // JOIN para traer info de la actividad y el paquete
    ...
    if (!error) setSelectedBookingItems(data || []);
  ...
  if (authLoading || isDataLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-stone-50"><Loader2 className="animate-spin w-10 h-10 text-orange-600" /></div>;
  }
    ...
    <div className="min-h-screen bg-stone-100/40 pb-20 relative">
          ...
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b flex justify-between items-center bg-stone-50/50">
              <div>
                ...
                <p className="text-xs text-stone-400 uppercase tracking-tighter">Desglose de actividades y servicios</p>
              </div>
            ...
            <div className="p-8 max-h-[70vh] overflow-y-auto">
              {loadingDetails ? (
                <div className="flex flex-col items-center py-20 gap-3">
                  ...
                  <p className="text-sm text-stone-400">Consultando base de datos...</p>
                </div>
              ) : (
                    ...
                    return (
                      <div key={idx} className="p-6 bg-stone-50 rounded-3xl border border-stone-200/60 relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-4">
                          ...
                          <Badge className="bg-white border-stone-200 text-stone-600 shadow-sm">
                            {item.pax_qty} {item.pax_qty === 1 ? 'Viajero' : 'Viajeros'}
                          </Badge>
                        </div>

                        <p className="text-sm text-stone-500 mb-4 leading-relaxed">{activity?.description}</p>
                        ...
                        <div className="grid sm:grid-cols-2 gap-4 mt-6 pt-6 border-t border-stone-200/50">
                          ...
                          <div className="text-right flex flex-col justify-end">
                            <p className="text-[10px] font-bold text-stone-400 uppercase">Subtotal Item</p>
            ...
            <div className="p-6 border-t bg-stone-50/50 flex justify-center">
              <Button onClick={() => setIsModalOpen(false)} className="px-10 bg-stone-800 hover:bg-black rounded-full transition-all">Regresar al Panel</Button>
            </div>
          </div>
        </div>
      )}
      ...
      <header className="bg-white/90 backdrop-blur-xl border-b border-stone-200 sticky top-0 z-50">
        <div className="container mx-auto px-4 lg:px-8 h-20 flex items-center justify-between">
          ...
          <Button variant="ghost" onClick={logout} className="text-red-500 hover:bg-red-50 gap-2 font-bold text-xs uppercase tracking-wider">
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-serif font-bold text-stone-800 flex items-center gap-2"><ShoppingBag className="text-orange-600"/> Ventas Reales</h2>
              <Badge variant="outline" className="border-stone-200 text-stone-400">Live Data</Badge>
            </div>
              ...
              {reservations.map((res) => (
                <Card key={res.id} className="border-none shadow-sm bg-white hover:ring-2 ring-orange-100 transition-all overflow-hidden group">
                  <div className="p-6 flex justify-between items-center">
                    <div>
                      <p className="font-bold text-stone-800">{res.customer_email}</p>
                    ...
                    <Button onClick={() => handleOpenDetails(res.id)} className="rounded-full bg-stone-100 text-stone-600 hover:bg-orange-600 hover:text-white transition-all">
                      <Eye className="w-4 h-4 mr-2" /> Ver Especificaciones
                    </Button>
                  </div>
                ...
                <Card key={quote.id} className={`border-none shadow-sm transition-all ${quote.status === 'attended' ? 'opacity-50 grayscale' : 'bg-white hover:shadow-md'}`}>
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <Badge className={quote.status === 'pending' ? 'bg-orange-100 text-orange-700 hover:bg-orange-100 border-none px-3 py-1' : 'bg-green-100 text-green-700 hover:bg-green-100 border-none'}>
                        {quote.status === 'pending' ? 'Pendiente' : 'Atendido'}
                      </Badge>
                    </div>
                    <div className="bg-stone-50 p-4 rounded-2xl text-sm text-stone-500 italic mb-4 border border-stone-100 leading-relaxed font-light">
                      <Info className="w-4 h-4 mb-2 text-stone-300" />
                      "{quote.special_requests || "El cliente no especificó requerimientos extra."}"
                    </div>
                    <div className="flex justify-between items-center pt-4 border-t border-stone-100">
                      ...
                      {quote.status === 'pending' ? (
                        ...
                        <p className="text-xs text-green-600 font-bold flex items-center gap-1"><CheckCircle2 className="w-4 h-4" /> Solicitud Procesada</p>
                      )}
                    </div>
                  </CardContent>
                </Card>
